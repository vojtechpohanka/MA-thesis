...
CEE <- CEE %>%
  mutate(
    macro_region = case_when(
      ...
      countryname %in% c("Czech Republic", "Hungary", "Slovakia", "Poland") ~
        "V4",
...
df_party <- CEE %>%
  ...
  mutate(
    uncod = peruncod,
    econ = sum(
      c_across(c(per401:per404, per406:per415, per703, per704)),
      na.rm = TRUE
    ),
    welf = sum(
      c_across(c(per405, per504, per505, per701, per702)),
      na.rm = TRUE
    ),
    ...
    nat_multi = sum(
      c_across(c(per601, per602, per607, per608, per705)),
      na.rm = TRUE
    ),
...
df_elect <- df_party %>%
  ...
  ungroup() %>%
  mutate(
    decade = paste0(floor(year / 10) * 10, "s") # add decade variable
  )
...
unzip("data/ESS_subset.zip", exdir = "data/ESS")

ess <- read_csv(
  "data/ESS/ESS1e06_7-ESS2e03_6-ESS3e03_7-ESS4e04_6-ESS5e03_5-ESS6e02_6-ESS7e02_3-ESS8e02_3-ESS9e03_2-ESS10-ESS10SC-ESS11-subset.csv"
)
...
unzip(
  "data/P_Data_Extract_From_World_Development_Indicators.zip",
  exdir = "data/WB"
)
...
unzip("data/wb_pop.zip", exdir = "data/WB_pop")
...
unzip("data/wb_gdp.zip", exdir = "data/WB_gdp")
...
# patrimonial
pat <- c("Estonia", "Latvia", "Lithuania", "Slovakia", "Romania", "Bulgaria")
...
merge_elect <- merge_elect %>%
  mutate(
    minority_legacy = case_when(
      ethnic_minorities == 1 ~ "significant ethnic minority",
      legacy == "National accomodative" & ethnic_minorities == 0 ~
        "national accomodative & homogeneous",
...
wb_merge <- wb_long %>%
  left_join(wb_pop_long, by = c("countryname", "year"))
...
merge_elect_imm <- merge_elect %>%
  left_join(cycle_averages, by = c("countryname", "year" = "year"))


# check

merge_elect_imm[
  merge_elect_imm$countryname == "Romania",
...
df_party_cl[
  df_party_cl$countryname == "Romania" & df_party_cl$year > 1990,
  c("countryname", "year", "net_migration_avg")
]
...
df_elect_cl <- merge_elect_imm %>%
  left_join(wb_gdp_long, by = c("year", "countryname"))

# check

df_elect_cl[
  df_elect_cl$countryname == "Poland" & df_elect_cl$year >= 1993,
...
cee_full <- left_join(
  cee_party,
  IDs,
  by = join_by(party == dataset_party_id.x),
...
cee_coded <- cee_full %>%
  mutate(
    farright = case_when(
      party %in% cee_faral_clean$dataset_party_id.x ~ 1,
      party == 81713 ~ 1,
      party == 97522 ~ 1,
...
df_ches1 <- df_ches %>%
  ...
  rename(ches_id = party_id, year_ches = year)
...
ches_ids <- link_table_ches %>%
  rename(party = dataset_party_id.x, ches_id = dataset_party_id.y) %>%
...
df_coded1 <- df_uncoded %>%
  mutate(
    family = case_when(
      ## code CHES family variable based on the MARPOR parfam variable
      parfam == 10 ~ 7,
      parfam == 20 ~ 6,
      parfam == 30 ~ 5,
      parfam == 40 ~ 3,
...
cmp_full_join_pgv <- cmp_w_pgv_ids %>%
  ...
  full_join(
    running_parties,
    by = c("country_iso3c", "cmp_edate", "pgv_party_id")
  )
...
cmp_full_join_pgv_filled %>%
  ...
  mutate(
    postmatch_cabinet_size = n_distinct(
      ifelse(pgv_cabinet_party == 1, pgv_party_name_short, NA),
      na.rm = TRUE
    ),
...
cmp_w_pty_govt_status <- cmp_full_join_pgv_filled %>%
  ...
  mutate(
    pgv_cabinet_party = ifelse(is.na(pgv_cabinet_party), 0, pgv_cabinet_party)
  ) %>%
  ...
  summarize(
    comment = case_when(
      cmp_n_pgv_ids == 0 ~
        "no matching party found within matching ParlGov cabinet configuration",
      cmp_n_pgv_ids == 1 ~
        sprintf(
...
df_party_gov <- df_party %>%
  left_join(
    cmp_gov,
    by = c("edate" = "cmp_edate", "party" = "cmp_party")
  )
...
country_name <- unique(df24$country[
  !df24$country %in% c("tur", "nor", "swi", "ice")
])
...
survey_means <- trend_24 %>%
  ...
  reframe(
    immigrate_mean = mean(immigrate_salience, na.rm = TRUE),
    galtan_mean = mean(galtan_salience, na.rm = TRUE)
  ) %>%
  mutate(
    iso = countrycode(
      country_full,
