...
df_agg_year <- df_agg_year %>%
  pivot_longer(cols = econ:religion,
               names_to = "issue",
               values_to = "salience") 
...
df_agg_year_recoded <- df_agg_year %>%
  ...
  mutate(issue_ordered = fct_reorder(issue, 
                                     desc(salience), 
                                     .fun = mean)
         )
...
facet_overall <- ggplot(df_agg_year_recoded, aes(x = year, y = salience*100)) +
  geom_line()+
  scale_x_continuous(
    limits = c(1993,2023),
...
ggsave("output_final/Fig_facet_overall.jpg", facet_overall, height = 7, width = 10)
...
df_group_means <- df_group_year_recoded %>%
  ...
  summarise(mean = mean(salience*100),
            sd = sd(salience*100),
            se = sd(salience*100)/sqrt(length(salience*100)),
             .groups = "drop"
  ) %>%
  mutate(
    issue_ordered = fct_reorder(issue, 
                                     desc(mean), 
                                     .fun = max)
    ) 
...
ggsave("output_final/Fig_party_points.jpg", overall_party, height = 7, width = 10)
...
facet_party <- ggplot() +
  stat_smooth(data = plot_df2, aes(x = month, y = salience*100, 
                                   group= party_group),
              geom = "line",
              color = "grey",
              linetype = "twodash",
              span = 0.1, se= F)+ 
  geom_smooth(data = plot_df1, aes(x = month, y = salience*100,
                                   linetype = "solid",
                                   color = group,
                                   fill = group), 
              se = F,
...
ggsave("output_final/Fig_facet_issue_party.png", plot = last_plot(), height = 7, width = 10)
